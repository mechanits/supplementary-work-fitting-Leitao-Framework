...
#import brazil,brazilsewer, brazilsewer_served, ibnetsewer, ibnetsewer_served, ibnetwater, ibnetwater_served, brazilwater, brazilwater_served
...
def load_data(filepath):
    ...
    """
    Loads two-column text data, treats first column as x and second as y.
    """
    df = pd.read_csv(filepath, delimiter=',', header=None)  # or adjust delimiter if needed
    x = df.iloc[:, 0].values
    y = df.iloc[:, 1].values
    return remove_zero_y(*sort_data(x, y))    


# Base directory for data folder
BASE_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
BASE_DATA_DIR = os.path.join(BASE_PATH, 'data')


# DATABASES dictionary, pointing to files and loading dynamically
DATABASES = {
    'brazil_sewerN': lambda: load_data(os.path.join(BASE_DATA_DIR, 'brazilsewer', 'N_sewer.txt')),
    'brazil_sewerM': lambda: load_data(os.path.join(BASE_DATA_DIR, 'brazilsewer_served', 'M_sewer.txt')),
    'brazil_waterN': lambda: load_data(os.path.join(BASE_DATA_DIR, 'brazilwater', 'N_water.txt')),
    'brazil_waterM': lambda: load_data(os.path.join(BASE_DATA_DIR, 'brazilwater_served', 'M_water.txt')),
    'ibnetsewerN': lambda: load_data(os.path.join(BASE_DATA_DIR, 'ibnetsewer', 'ibnet2015Ntext.txt')),
    'ibnetsewerM': lambda: load_data(os.path.join(BASE_DATA_DIR, 'ibnetsewer_served', 'ibnetsewer2015Mtext.txt')),
    'ibnetwaterN': lambda: load_data(os.path.join(BASE_DATA_DIR, 'ibnetwater', 'ibnetwater2015Ntext.txt')),
    'ibnetwaterM': lambda: load_data(os.path.join(BASE_DATA_DIR, 'ibnetwater_served', 'ibnetwater2015Mtext.txt')),
}
...
#             'brazil_externalCauses_2010': lambda: remove_zero_y(*sort_data(*brazil.externalCauses(2010))),'brazil_sewerN':lambda: remove_zero_y(*sort_data(*brazilsewer.index())), 
#	     'brazil_sewerM':lambda: remove_zero_y(*sort_data(*brazilsewer_served.index())), 
#	     'brazil_waterN':lambda: remove_zero_y(*sort_data(*brazilwater.index())), 
#	     'brazil_waterM':lambda: remove_zero_y(*sort_data(*brazilwater_served.index())),
#	     'ibnetsewerN':lambda: remove_zero_y(*sort_data(*ibnetsewer.index())), 
#	     'ibnetsewerM':lambda: remove_zero_y(*sort_data(*ibnetsewer_served.index())),
#	     'ibnetwaterN':lambda: remove_zero_y(*sort_data(*ibnetwater.index())), 
#	     'ibnetwaterM':lambda: remove_zero_y(*sort_data(*ibnetwater_served.index())),
#             }
class XYData:
...
def xy(dataset_name):
    """
    Returns XYData object for a given dataset name.
    """
    if dataset_name not in DATABASES:
        raise ValueError(f"Dataset '{dataset_name}' not found.")
    x, y = DATABASES[dataset_name]()
    return XYData(x, y)
...
class FixedDAnalysis(LikelihoodAnalysis):
    description = r'Gaussian fluctuations with \delta = 1'
...
class FixedDFixedBetaAnalysis(LikelihoodAnalysis):
    description = r'Gaussian fluctuations with \beta = 1 and \delta = 1'
    def __init__(self, dataset, required_successes=16):
...
class PopulationFixedGammaAnalysis(PopulationAnalysis):
    ...
    def __init__(self, dataset, required_successes):
    	...
    	# Remove the cache decorator and call the _mle_calculation directly
        result = self._mle_calculation(self._cache_name)
    ...
    def _mle_calculation(self, cache_file):
        """
        Compute the MLE calculation without caching.
        """
        model = self.Model(self.bounds)
...
class LogNormalFixedDAnalysis(LogNormalAnalysis):
    description = r'Log normal fluctuations with \delta = 2'
    def __init__(self, dataset, required_successes=16):
...
class LogNormalFixedDFixedBetaAnalysis(LogNormalAnalysis):
    description = r'Log normal fluctuations with \beta = 1 \delta = 2'
    def __init__(self, dataset, required_successes=16):
