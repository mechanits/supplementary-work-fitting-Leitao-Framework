...
def minimize_with_errors(model, x, y, parameters=None, samples=200, disp=True):
    """
...
class NormalModel(object):
    ...
    def get_random_parameters(self, x, y):
        ...
        d = self.bounds[3][0] + (self.bounds[3][1] - self.bounds[3][0])*random() #0.5 #+ #1.0*random()                    # [0.5, 1.5]
...
class LogNormalModel(NormalModel):
    ...
    def get_random_parameters(self, log_x, log_y):
        ...
        c = 5.0*random()                        # [0, 5]
        c = self._check_initial(c, self.bounds[2])
        d = self.bounds[3][0] + (self.bounds[3][1] - self.bounds[3][0])*random() #1 #+ #2.0*random()                      # [1, 3]
        d = self._check_initial(d, self.bounds[3])
